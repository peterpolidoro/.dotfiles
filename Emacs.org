...
**GNU Guix**. It tangles three files:

- =.emacs.d/early-init.el=  (pre-GUI/startup tuning)
- =.emacs.d/init.el=        (main configuration)
- =./emacs.scm=             (Guix manifest containing Emacs packages + tools + Tree‑sitter grammars)

To tangle everything: =C-c C-v t= (or =M-x org-babel-tangle=) from this buffer.

* early-init.el  (tangled to =.emacs.d/early-init.el=)
:PROPERTIES:
:header-args:emacs-lisp: :tangle ./.emacs.d/early-init.el :mkdirp yes :results silent
:END:
